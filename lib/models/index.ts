...
import { Endpoint, endpointSchema, endpoints } from "../endpoints";
import { embeddingModels, validateEmbeddingModelByName } from "../embedding-model";

import type { PreTrainedTokenizer } from "@xenova/transformers";

import JSON5 from "json5";
import { getTokenizer } from "@/lib/utils/get-tokenizer";
import { ToolResultStatus } from "../types/tool";
import { logger } from "../logger";
import { sum } from "../utils/sum";
import { endpointTgi } from "../endpoints/tgi";
...
const modelsRaw = z.array(modelConfig).parse(JSON5.parse(process.env.MODELS ?? "[]"));
...
const addEndpoint = (m: Awaited<ReturnType<typeof processModel>>) => ({
	...
	getEndpoint: async (): Promise<Endpoint> => {
		if (!m.endpoints) {
			return endpointTgi({
				type: "tgi",
				url: `${process.env.HF_API_ROOT}/${m.name}`,
				accessToken: process.env.HF_TOKEN ?? process.env.HF_ACCESS_TOKEN,
				weight: 1,
				model: m,
			});
		}
...
export const oldModels = process.env.OLD_MODELS
	? z
			.array(
				z.object({
					id: z.string().optional(),
			...
			.parse(JSON5.parse(process.env.OLD_MODELS))
...
export const smallModel = process.env.TASK_MODEL
	? (models.find((m) => m.name === process.env.TASK_MODEL) ||
			(await processModel(modelConfig.parse(JSON5.parse(process.env.TASK_MODEL))).then((m) =>
				addEndpoint(m)
			))) ??
	  defaultModel
	: defaultModel;
